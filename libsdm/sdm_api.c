...
void recover_misspec(long, pid_t *, struct SDM);
void *scan(void *);
void *decompress(void *);
void merge(pid_t*, struct SDM);
...
void recover_misspec(long pos, pid_t *pids, struct SDM conf) {
	int i, pid = 0;
	pid_t rec;
	unsigned int len;
...
inline void merge(pid_t *pids, struct SDM conf) {
...
void sdm_store_buf(unsigned char *buf, int len, int buffer_index) {
